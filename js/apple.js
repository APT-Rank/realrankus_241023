    ...
    if(agreementChecker()){
      ...
      if(connectionWebApp == "App"){
        if(connectionOS = "iOS"){
          ...
          appleSignInPopup(provider) //2024-01-20
      }
    }
}
...
function getAppleUserInfo() {
  const AppleUser = firebase.auth().currentUser;
  if (checkMobile() == "ios"){
    alert("Apple User : " + AppleUser)
  }
  ...
  firebase
    ...
    .then((result) => {
      ...
      if(user && login_status == false){
        ...
        setLogoutStatus() //2024-01-20
        //appleReauthenticateWithRedirect()
      }
    })
    .catch((error) => {      
      // Handle Errors here.
