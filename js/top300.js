function showLoginRequest(){
  ...
  load_parrtnership(selectedSubRegion);      
  ...
  detailHtml += "<div><br></div>"
  detailHtml += "<div id='aptrankLoginGuide'>로그인 하시면 상세 정보 확인이 가능해요</div>"
  detailHtml += "<div id='aptrankLoginButton' data-bs-toggle='modal' data-bs-target='#loginModal' id='loginPopup' onClick='showLogin()'>리얼랭커스 로그인</div>"
  detailHtml += "</div>"
  $('#dataList').append(detailHtml)
}
...
function updateTopTable(month, region){
  ...
  if(Number(selectedMonth) < 202207){    
    ...
    url = "https://www.realrankus.com/" + month + "/" + region + ".json"
    $('#dataList').html("");
    $('body').append("<div id='pageLoadingBack'><div class='spinner-grow text-pageLoading' role='status'></div><div style='font-size: 0.85em; color: white'><br>지역 정보를 불러오고 있어요!</div></div>")  
    ...
    */
    ...
    .done(function(){
        ...
        for(var i = 0 ; i < itemNum ; i++){
          ...
          top_300_data.push({"아파트명":aptName, "법정동주소":aptAddress2, "검색코드":searchCode, "lat":coord_y, "lng":coord_x, "last_sales": aptData.data[i]["last_sales"], "rank":rank})
          ...
          if(checkPrice(last_sales[1])){
            ...
            if(selectedSubRegion == "Trans_Top300" && i == 0){
              var addon_html = "<div class='content' style='background-color: #ffdfde; border-bottom: 1px solid gray; padding-top: 0.5em; padding-bottom: 0.5em; margin-top: 5px'><div class='apt_name' style='text-align:center; font-size: 0.75em;'> '출퇴근 교통'은 지하철이 있는 수도권과 광역시만 선정하고 있어요.</div></div>"
              addon_html += "<div class='listBox' data-bs-toggle='modal' data-bs-target='#baseModal' id='aptSelect_" + i + "' onClick='showTopDetail(" + i + ")'>";
            }
            else{
              var addon_html = "<div class='listBox' data-bs-toggle='modal' data-bs-target='#baseModal' id='aptSelect_" + i + "' onClick='showTopDetail(" + i + ")'>";
            }
        ...
        //지도에 아이콘 표시
        top_300_data = top_300_data.reverse()
        if(!defaultMap){
          setTimeout(function(){
            if(defaultMap){        
              defaultMap.setCenter(top_300_data[top_300_data.length-1])
...
function showTopDetail(index){
  ...
  */
  ...
  shareText += "\n" + "https://www.realrankus.com" + "?reg=" + selectedRegion +"&sub=" + selectedSubRegion + "&mon=" + selectedMonth + "\n"
  ...
  if(mapBounds.hasLatLng(target_position)){
    if(current_zoom >= zoom_levels[0]){      
      marker_obj = window["top_marker_obj_" + searchCode]
    }
  ...
  offsetY = $(find_id).offset().top - table_offset_y + 71
...
function eduCard(aptData, index){
  ...
  var mSchool_edu = aptData.data[index]["중학교학업성취도"] + "%"
  var academy_edu = aptData.data[index]["500m이내학원가"] + "개" + "(총 " + aptData.data[index]["500m이내학원수"] + "개 학원)"
  if(Number(selectedMonth) > 202308){
    var academy_edu_long = aptData.data[index]["1km이내학원가"] + "개" + "(총 " + aptData.data[index]["1km이내학원수"] + "개 학원)"
  }
  var drink_pub = aptData.data[index]["300m이내유흥주점"]
  var daran_pub = aptData.data[index]["300m이내단란주점"]
  ...
  detailHtml += "<div class='popSubTable'><div class='popContent'>" + "300m 이내 학원가" + "</div>" + "<div class='popResult'>" + academy_edu + "</div></div>";
  if(Number(selectedMonth) > 202308){
    detailHtml += "<div class='popSubTable'><div class='popContent'>" + "1km 이내 학원가" + "</div>" + "<div class='popResult'>" + academy_edu_long + "</div></div>";
  }
